...
import { RoadmapItem, RoadmapItemKR, ROADMAP_COLORS, formatDateOnly, parseDateOnly, getQuarterLimits, formatQuarterRange, isDateInQuarter } from '@/types/roadmap';
...
const EditRoadmapDialog = ({ item, objectives, existingThemes = [], open, onOpenChange, onSave }: Props) => {
  ...
  const [category] = useState<OKRCategory>(item.category);
  ...
  const quarterOptions = useMemo(() => buildQuarterOptions(item.quarter), [item.quarter]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg p-0 flex flex-col max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
        <DialogHeader className="flex-shrink-0 p-6 pb-0"><DialogTitle>Editar Iniciativa</DialogTitle></DialogHeader>
        ...
        <div className="flex-shrink-0 p-6 pt-4 border-t border-white/10 flex gap-3 justify-end">
